...
plot_year_subgroup_bars <- function(
    ...
    legend = TRUE,
    title = NULL,
    show_labels = TRUE,     # NEW
    axis_percent = FALSE,   # optional % axis labels
    text_percent = FALSE,   # optional % on text labels
    text_digits = 1         # decimals for text labels
) {
  ...
  data <- data |>
    dplyr::mutate(year_label = factor(as.character(year), levels = c("2000", "2019")))
  
  p <- ggplot2::ggplot(data, ggplot2::aes(x = subgroup, y = {{ yvar }})) +
    ggplot2::geom_bar(
      ggplot2::aes(fill = year_label, color = year_label, linetype = year_label),
      stat = "identity",
  ...
  if (show_labels) {
    label_fun <- function(x) {
      base <- round(x, text_digits)
      if (text_percent) paste0(base, "%") else as.character(base)
    }
    p <- p +
      ggplot2::geom_text(
        ggplot2::aes(label = label_fun({{ yvar }}), group = year),
        position = ggplot2::position_dodge(width = 0.8),
        vjust = -0.5,
        size = 3
      )
  ...
  p <- p +
    ggplot2::scale_fill_manual(
      values = c(
        "2000" = scales::alpha(bar_fills$per1$color, bar_fills$per1$alpha),
        "2019" = scales::alpha(bar_fills$per2$color, bar_fills$per2$alpha)
      ),
      guide = if (legend) "legend" else "none"
    ) +
    ggplot2::scale_color_manual(values = border_colors, guide = if (legend) "legend" else "none") +
    ggplot2::scale_linetype_manual(values = line_types, guide = if (legend) "legend" else "none") +
    ggplot2::labs(y = ytitle, title = title, fill = NULL, color = NULL, linetype = NULL) +
  ...
  if (!is.null(ymin) || !is.null(ymax)) {
    p <- p + ggplot2::coord_cartesian(ylim = c(ymin %||% -Inf, ymax %||% Inf))
  }
...
make_observed_cf_barplot <- function(row, yaxis_override = NULL, label_format = "%.3f") {
  ...
  ggplot(fig_data, aes(x = Category, y = Household_Size, fill = Type, linetype = Type)) +
    ...
    geom_text(aes(label = sprintf(label_format, Household_Size)), vjust = 1.5, color = "white", linewidth = 4) +
